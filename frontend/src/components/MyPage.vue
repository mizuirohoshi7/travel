      ...
      <div class="liked-plans">
        ...
        <ul>
          <li v-for="plan in likedPlans" :key="plan.id">
            {{ plan.location }} - {{ formatDate(plan.travelDate) }}
          </li>
        </ul>
      </div>
...
export default {
  ...
  data() {
    return {
      tokenAmount: 0,
      likedPlans: [],
      followingList: [], // Holds the list of followed users
    };
  },
  methods: {
    async fetchData() {
      try {
        ...
        const [memberResponse, likeResponse, followResponse] = await Promise.all([
          ...
          axios.get(`/follows?fromId=${memberId}`), // Fetch following users
        ]);
        ...
        this.followingList = userResponses.map(response => response.data); // List of followed users
      } catch (error) {
        console.error('데이터 가져오기 실패:', error);
      }
    },
    goToTokenCharge() {
      // Redirect to token charge page
